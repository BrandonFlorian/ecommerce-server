...
export const addItemToCart = async (cartId: string, itemData: CartItemDto) => {
  try {
    ...
    const { data: existingItem, error: itemError } = await supabaseClient
      .from("cart_items")
      .select("id, quantity")
      .eq("cart_id", cartId)
    ...
    if (existingItem) {
      ...
      const { data: newItem, error: addError } = await supabaseClient
        .from("cart_items")
        .insert([
          {
            cart_id: cartId,
